...
def sendEvents(vrnisession, vrosession, fromTime=None, toTime=None):
	...
	event_search_query = "/api/search/query?searchString=" + urllib.quote_plus("open problems where Alert Name = 'nsx-t edge node controller connectivity is 'Unknown' and timestampinms >= " + str(fromTime) + " and timestampinms <= " + str(toTime)) +\
  ...
  "parameters": [
       {
		...
		responseSlack = vrosession.post(vrourl + "/vco/api/workflows/9052a4a1-8ccd-4eb6-bfda-a97218e0f5f9/executions", data=finalobject, verify=False,
